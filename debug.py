'''
For debugging the call:
python inference.py \
    --exp_idx=1 \
    --cuda=0 \
    --input_dir=input_drumless \
    --output_dir=output_with_drums 
    --sample_iters=10 \

# Default args from inference.py are:
...
exp_idx = 1 # the best one going by the readme.md

input_dir = "input_drumless"
output_dir = "output_with_drums" #os.path.join(args.output_dir, f'exp{exp_idx}')

sample_iters = 10

temp = 0.96 # provided default
top_p = 0.8 # provided default

hps = setup_lm_hparams(MODEL_LIST[exp_idx])
hps.cuda = 0 # if using GPU
hps.batch_size = 1
...
print(f'temp: {temp}')
print(f'generate samples from {input_dir}')
print(f'save generated samples to {output_dir}')
...
for j in range(sample_iters):
    for i in tqdm(range(len(dataset))):
        ...
        with torch.no_grad():
            lm.eval()
            gen_mel = lm.sample(n_samples=hps.batch_size, otz=otz, binfo=binfo, vqvae=target_vqvae, temp=temp, top_p=top_p)
            gen_mel = gen_mel * target_std + target_mean 
            gen_wavs = vocoder(gen_mel)
        orig, _ = sf.read(os.path.join(input_dir, fn),always_2d=True)
